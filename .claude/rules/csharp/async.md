...
## The game loop is synchronous
- The render/update loop runs synchronously on the main thread — OpenGL calls must happen on the thread that owns the GL context. Do not make per-frame `Update`/`Render` methods `async`, and do not `await` inside the loop.
- Use async for genuinely I/O-bound, off-the-hot-path work: loading assets (shaders, textures, models) from disk at startup or during a load screen.
